...
Status: implemented in d4c5fea; unsigned installer built, not installed or
published. Independent review on 2026-09-09 requires a fix for the legacy
read-only Git launch's inherited frozen DLL state, plus an actual frozen-Brain
PDF-spawn integration check. Fix round 1 is in progress; final privacy/native
acceptance and rebuild remain open.
...
Independent review requires NFC normalization for decomposed-accent queries
and recommends omitting last-retrieval mode before the first search. Follow-up
fixes and scoped review remain pending.
