...
**Applied through 2026-09-18:** retired `AUDIT_PLAN.md`, `DEEPSCAN_AUDIT.md`, `PHASE3_READINESS_AUDIT.md`, `PONYTAIL_DEEPSCAN_2026-08-02.md`, completed Phase 3 UI/approval/batched-task plan-spec pairs, the dependency-framework audit and execution plan, and `.claude/commands/write-test.md` (obsolete runner guidance). Live requirements were moved into the canonical design, roadmap, verification, and memory documents before deletion. Git history remains the forensic archive.
...
**B3 — the checkpoint stays the sole source of truth for pending approvals; rehydrate is scoped by a narrowing adapter query, NOT by a persisted set in halo.db.** `CheckpointStore.open_interrupt_threads()` selects only threads whose latest checkpoint carries an `__interrupt__` write instead of deserializing every thread ever created. `graph.py` accesses this through the checkpoint compatibility adapter and never reaches into saver SQL directly. **Why not persist "threads with an open approval" to halo.db and skip the checkpoint scan entirely?** Because the approval is written into the LangGraph checkpoint by the graph invocation, and our own set would be a second write — a crash in the window between the two would silently lose an approval. The query only narrows candidates; the snapshot interrupt remains the payload authority. **Also deliberately NOT done: deleting/archiving whole dormant threads' checkpoints.** No reliable "conversation is closed" signal exists, per-thread depth is already capped, and archival would trade resumable history for disk. Revisit only with an explicit user-driven delete-conversation action.
...
## Source sidecars in development; frozen backend in releases — 2026-07-10, updated 2026-09-18
**What:** Development spawns `python -m brain` / `python -m voice` for debuggability. Release packaging installs one PyInstaller-built `halo-backend` external binary and selects its Brain, Voice, or PDF-worker mode.
**Why:** the source path preserves fast development while the frozen path proves users do not need a developer Python installation. Both modes implement the same IPC and supervision contract.
**Trade-off:** source and release discovery paths both require tests; `packaging/README.md` and the Rust supervisor tests are the maintained evidence.
...
**Superseded by the fixed 360×52 companion capsule.** This entry records the retired orb-only implementation and is not current UI guidance; see `ui_ux/01-companion-orb.md`.
