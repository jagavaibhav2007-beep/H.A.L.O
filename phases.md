...
4. **Memory** — 3 tiers (session / curated beliefs / raw log), SQLite FTS5/BM25 baseline with optional sqlite-vec + local embeddings, decay + auto-correct; wired to the memory panel ([03-memory](systemdesign/03-memory.md)).
...
- **3c — Voice.** The frozen full-profile `halo-backend` packaging foundation, including optional fastembed/sqlite-vec loading, is implemented. Next add the Pipecat pipeline, openWakeWord, local faster-whisper STT, local Kokoro TTS, cloud fallback, barge-in, narration of main events, and "stop" → redirect → resume ([02-voice](systemdesign/02-voice.md)).
