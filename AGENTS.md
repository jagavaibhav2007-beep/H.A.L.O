...
**Current working situation (2026-09-18).** Phase 0-2 work and the Phase 3 UI foundation are merged to `main`; the Phase-3a managed-command foundation and dependency/packaging foundation are implemented on the dependency-audit feature branch. Always inspect `git status` and the relevant `git diff` before editing and preserve unrelated work. Codex/Claude adapters are the next 3a tranche; real voice remains the next 3c capability, and per-client `stream_frame` subscription remains scheduled with 3d.
...
**Task runtime.** Task-shaped tools detach from the short interactive-turn pool and per-conversation lock into the bounded, durable `TaskRuntime` (`HALO_TASK_CONCURRENCY`, default 2). Its intent/result columns were introduced in schema v5 (the current store is v6), it gives tools cooperative stop/pause/progress/log callbacks, emits bounded `task_log` tails to UI clients, and reconciles torn tasks after restart without blindly replaying side effects. `dir_organize`, `doc_digest`, `command_run`, and `script_run` are task-shaped tools. `task_op` is real; unsupported pause is reported as a correlated error. Read `systemdesign/12-task-runtime.md` before adding long-running work.
...
## Repo conventions (source-visible; project license deferred)
...
- Phase 2 — COMPLETE (declared 2026-08-01). Steps 1–10 plus exit hardening are implemented: durable tasks (`TaskRuntime`; metadata introduced in schema v5, current store v6), authority separation, bounded admission, atomic batch undo, turn correlation, project-root repair, dependency locks, scoped checkpoint rehydration, Voice reconnect, and bounded UI turns. The unchecked native scenarios in `VERIFY.md` remain recommended, non-blocking follow-ups.
- Phase 3 — underway. The Phase-3a managed-command foundation and frozen-backend packaging foundation are implemented; Codex/Claude adapters are next. Read `systemdesign/05-computer-control.md`, `systemdesign/07-coding-orchestration.md`, and `systemdesign/12-task-runtime.md` before extending it. Real audio/wake/STT/TTS work remains for 3c, and per-client `stream_frame` subscription with 3d remains a just-in-time prerequisite in `phases.md`.
